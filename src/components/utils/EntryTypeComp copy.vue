    ...
    >
      ...
      <div class="row items-center justify-between text-caption q-mb-xs">
        <q-chip
          dense
          size="sm"
          :color="entryData.drEffect === '+' ? 'positive' : 'negative'"
          text-color="white"
        >
          Dr: {{ entryData.drEffect }}
        </q-chip>
        <div class="text-right text-weight-bolder text-subtitle2 text-primary ellipsis">
          {{ formatCurrency(entryData.totalDebit) }}
        </div>
      </div>
      ...
      <div class="row items-center justify-between text-caption">
        <q-chip
          dense
          size="sm"
          :color="entryData.crEffect === '+' ? 'positive' : 'negative'"
          text-color="white"
        >
          Cr: {{ entryData.crEffect }}
        </q-chip>
        <div class="text-right text-weight-bolder text-subtitle2 text-primary ellipsis">
