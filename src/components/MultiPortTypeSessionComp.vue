    ...
    <q-card flat class="bg-body text-appText q-pa-sm fit column no-wrap overflow-hidden">
      ...
      >
        <q-card
          ...
          :key="`${item.portType}-${item.sessionType}`"
          flat
          bordered
          ...
          <div class="row items-center justify-between no-wrap q-gutter-x-xs min-width-0">
            <div class="row items-center no-wrap col min-width-0">
              <q-icon
                ...
                :name="getSessionTypeDescription(Number(item.portType), item.sessionType).iconName"
              />
              <span
                class="text-weight-bold text-caption text-appText ellipsis"
                :title="
                  getSessionTypeDescription(Number(item.portType), item.sessionType).description
                "
              >
                {{ getSessionTypeDescription(Number(item.portType), item.sessionType).description }}
              </span>
...
import { MultiPortTypeSummary } from '../types/myTypes.js'
import { formatCurrency, getSessionTypeDescription, getSessionEffect } from '../modules/appUtils.js'

export default defineComponent({
  name: 'MultiPortTypeSessionComp',
  props: {
    sessionTypeSummaries: {
      type: Array as PropType<MultiPortTypeSummary[]>,
      required: false,
      default: (): MultiPortTypeSummary[] => []
    }
  },
...
/* CSS Grid layout replacing Quasar flex columns */
.session-grid {
  display: grid;
